...
import type { Character, Business, InsertCharacter } from '../schema.js';
import {
  MAIN_QUEST_NPC_DEFINITIONS,
  MAIN_QUEST_NPC_TAG,
  type MainQuestNPCRole,
  type MainQuestNPCDefinition,
} from '../quest/main-quest-npcs.js';
...
export async function ensureMainQuestRoles(
  ...
  const currentMap = await getMainQuestNPCs(storage, worldId);
...
export async function getMainQuestNPCNames(
  ...
  const npcMap = await ensureMainQuestRoles(storage, worldId);
