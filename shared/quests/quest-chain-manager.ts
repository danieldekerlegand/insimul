...
export class QuestChainManager {
  ...
  async canAcceptQuest(quest: Quest, playerId: string): Promise<boolean> {
    ...
    const playerQuests = await this.storage.getQuestsByPlayer(playerId);
  ...
  async getNextQuestInChain(currentQuest: Quest): Promise<Quest | null> {
    ...
    const allQuests = await this.storage.getQuestsByWorld(currentQuest.worldId);
  ...
  async reorderQuestChain(
    ...
    for (let i = 0; i < questOrder.length; i++) {
      ...
      await this.storage.updateQuest(questId, {
        questChainOrder: i,
      });
    }
  }
  ...
  async addQuestToChain(
    ...
    const quest = await this.storage.updateQuest(questId, {
      questChainId: chainId,
  ...
  async checkChainCompletion(quest: Quest): Promise<ChainCompletionResult> {
    ...
    const allQuests = await this.storage.getQuestsByWorld(quest.worldId);
...
// Factory: create a QuestChainManager with the given storage provider
export function createQuestChainManager(storage: QuestStorageProvider): QuestChainManager {
  return new QuestChainManager(storage);
}
