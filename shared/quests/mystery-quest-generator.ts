...
/** Minimal Prolog engine interface for mystery quest generation */
export interface PrologEngine {
  query(goal: string, maxResults?: number): Promise<Array<Record<string, string>>>;
}

/** Prolog sync interface — provides engines per world */
export interface PrologAutoSyncProvider {
  getEngine(worldId: string): PrologEngine | null;
  isWorldSynced(worldId: string): boolean;
}

/** Must be set before calling generateMysteryQuest */
let prologAutoSync: PrologAutoSyncProvider | null = null;

export function setPrologProvider(provider: PrologAutoSyncProvider): void {
  prologAutoSync = provider;
}
