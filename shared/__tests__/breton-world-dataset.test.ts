...
const BRETON_DIR = join(__dirname, '../../data/worlds/language/breton');

function readPrologFile(filename: string): string {
  return readFileSync(join(BRETON_DIR, filename), 'utf-8');
}
...
describe('Breton World Dataset', () => {
  ...
  it('should have all required files', () => {
    const files = readdirSync(BRETON_DIR).filter((f) => f.endsWith('.pl'));
  ...
  describe('world.pl', () => {
    ...
    it('should set world_type to historical_medieval', () => {
      const content = readPrologFile('world.pl');
      expect(content).toContain('world_type(medieval_breizh, historical_medieval)');
    });

    it('should set target_language to breton', () => {
      const content = readPrologFile('world.pl');
      expect(content).toContain('target_language(medieval_breizh, breton)');
    });
  });
  ...
  describe('items.pl', () => {
    ...
    it('should have culturally specific Breton items', () => {
      const content = readPrologFile('items.pl');
      expect(content).toContain('sistr');
      expect(content).toContain('koef');
      expect(content).toContain('krampouezh');
    });
  });

  describe('languages.pl', () => {
    it('should define Breton language', () => {
      const content = readPrologFile('languages.pl');
      expect(countPredicates(content, 'language')).toBeGreaterThanOrEqual(1);
      expect(content).toContain("language(breton, 'Brezhoneg')");
      expect(content).toContain('language_learning_target(breton)');
    });
  });
