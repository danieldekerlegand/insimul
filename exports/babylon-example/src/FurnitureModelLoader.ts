...
export class FurnitureModelLoader {
  ...
  public async loadAll(): Promise<void> {
    ...
    // Skip all furniture model loading in exported games — polyhaven models are not bundled
    console.log('[FurnitureModelLoader] Skipping furniture model loading (not bundled in export)');
    this.loading = false;
    return;
  ...
  private async loadTemplate(type: string, relativePath: string): Promise<void> {
    try {
      // Skip models that aren't bundled in the export (polyhaven assets are large and not included)
      if (relativePath.includes('polyhaven')) return;
