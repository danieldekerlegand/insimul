...
export class FileDataSource implements DataSource {
  ...
  async loadSettlementBusinesses(settlementId: string): Promise<any[]> {
    await this.waitForData();
    // Derive businesses from buildings data (buildings with a businessId are businesses)
  ...
  async loadSettlementResidences(settlementId: string): Promise<any[]> {
    await this.waitForData();
    // Derive residences from buildings data (buildings without a businessId are residences)
    const buildings = (this.worldData?.buildings || []).filter(
      (b: any) => b.settlementId === settlementId && !b.businessId
    );
    return buildings.map((b: any) => ({
      ...
      lotId: b.lotId,
      position: b.position,
      ownerIds: b.occupantIds?.slice(0, 1) || [],
      residentIds: b.occupantIds || [],
    }));
  }
