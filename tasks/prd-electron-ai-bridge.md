...
**Acceptance Criteria:**
- [ ] Create `electron/ai-service.js` (or `.ts` compiled to JS) that exports an `initAIService(app)` function
- [ ] On startup, reads `dist/data/ai_config.json` to determine if `apiMode === 'local'`
- [ ] When local, loads node-llama-cpp with the GGUF model from the path in `ai_config.json` (default: `ai/models/phi-4-mini-q4.gguf` relative to app root)
- [ ] GPU auto-detection: attempts Metal (macOS) or CUDA (Linux/Windows), falls back to CPU
- [ ] Model loads once and stays in memory — subsequent IPC calls reuse it
- [ ] Registers IPC handlers: `ai:generate`, `ai:generate-stream`, `ai:tts`, `ai:stt`, `ai:status`
- [ ] If model loading fails, logs the error and sets `aiAvailable = false` (game continues without AI)
- [ ] Concurrent request queuing — node-llama-cpp is single-threaded per model, so requests are serialized
- [ ] `electron/main.js` calls `initAIService(app)` during app startup
- [ ] Typecheck passes
...
**Acceptance Criteria:**
- [ ] Piper voice model loaded from `ai/models/voices/` directory relative to app root
- [ ] `ai:tts` IPC handler accepts `{ text: string, voice?: string, speed?: number }` and returns `ArrayBuffer` (WAV or MP3)
- [ ] Maps voice names (Kore, Charon, Aoede, Puck) to Piper voice model files
- [ ] Gender selection (male/female/neutral) via voice model selection
- [ ] Returns audio within ~500ms for a typical NPC sentence (real-time or faster on modern hardware)
- [ ] Graceful failure: returns null if Piper is not available, caller falls back to text-only
- [ ] Typecheck passes
...
**Acceptance Criteria:**
- [ ] Whisper model loaded from `ai/models/whisper-base.bin` (or path from `ai_config.json`)
- [ ] `ai:stt` IPC handler accepts `ArrayBuffer` (WebM/WAV audio from recorder) and returns `{ text: string, language?: string }`
- [ ] Converts input audio to 16kHz WAV internally if needed
- [ ] Language hint support (BCP-47 codes) matching existing STT interface
- [ ] Auto-language detection when no hint provided
- [ ] Returns transcription within ~2 seconds for a typical player utterance
- [ ] Graceful failure: returns empty string if Whisper is not available
- [ ] Typecheck passes
...
**Acceptance Criteria:**
- [ ] `electron/preload.js` exposes `window.electronAPI.aiAvailable` (boolean, set after model load)
- [ ] Exposes `window.electronAPI.aiGenerate(prompt, options)` → `Promise<string>`
- [ ] Exposes `window.electronAPI.aiGenerateStream(prompt, options, onChunk)` → registers an IPC listener that calls `onChunk(token)` for each token, returns `Promise<string>` with full text
- [ ] Exposes `window.electronAPI.aiTTS(text, voice, speed)` → `Promise<ArrayBuffer | null>`
- [ ] Exposes `window.electronAPI.aiSTT(audioBuffer, languageHint)` → `Promise<{ text: string }>`
- [ ] Exposes `window.electronAPI.aiStatus()` → `Promise<{ loaded: boolean, modelName: string, gpuLayers: number, gpuType: string }>`
- [ ] All IPC calls use `contextBridge.exposeInMainWorld` (no `nodeIntegration`)
- [ ] Typecheck passes
...
**Acceptance Criteria:**
- [ ] Create `src/LocalAIClient.ts` with methods: `generate(prompt, systemPrompt?, options?)`, `generateStream(prompt, systemPrompt?, options?, onToken?)`, `textToSpeech(text, voice?)`, `speechToText(audioBlob)`
- [ ] Static `isAvailable()` method that checks `window.electronAPI?.aiAvailable === true`
- [ ] `generateStream` calls `window.electronAPI.aiGenerateStream` and invokes `onToken` callback for each token received
- [ ] `generate` is a convenience wrapper that accumulates all tokens from `generateStream`
- [ ] `textToSpeech` returns an `AudioBuffer` or `Blob` ready for `StreamingAudioPlayer`
- [ ] `speechToText` converts the browser `Blob` to `ArrayBuffer` and sends via IPC
- [ ] If not in Electron or AI not available, all methods throw a clear error (callers should check `isAvailable()` first)
- [ ] Typecheck passes
...
**Acceptance Criteria:**
- [ ] `ConversationClient` constructor accepts optional `localAI?: LocalAIClient`
- [ ] `sendText()`: when local AI available, builds a system prompt from NPC personality/context and calls `localAI.generateStream()` with `onToken` mapped to `onTextChunk` callback
- [ ] `sendText()`: on completion, calls `localAI.textToSpeech()` for audio and emits `onAudioChunk`
- [ ] `sendAudio()`: when local AI available, calls `localAI.speechToText()` first, then routes the transcribed text through `sendText()` logic
- [ ] State transitions (thinking → speaking → idle) match the existing HTTP SSE flow
- [ ] `onComplete` callback fires with full accumulated text
- [ ] Existing HTTP SSE path preserved as fallback when `localAI` is null/unavailable
- [ ] Typecheck passes
...
**Acceptance Criteria:**
- [ ] In `BabylonGame.ts`, the `onStartConversation(npc1Id, npc2Id, topic)` callback checks `LocalAIClient.isAvailable()`
- [ ] When available, builds a system prompt describing both NPCs (name, personality traits, occupation, relationship) from loaded character data
- [ ] Generates 2-4 exchange pairs via `localAI.generate()` with a structured output prompt requesting JSON
- [ ] Parses the LLM response into `ConversationResult` shape: `{ exchanges, relationshipDelta, topic, languageUsed }`
- [ ] Falls back to returning `null` if local AI is not available or generation fails
- [ ] NPC conversations feel natural and reflect personality (not generic filler)
- [ ] Typecheck passes
...
**Acceptance Criteria:**
- [ ] During `BabylonGame.init()`, after engine initialization, check `LocalAIClient.isAvailable()` and show "Loading AI model..." in the loading screen
- [ ] Query `window.electronAPI.aiStatus()` and display model name and GPU type (e.g., "AI: Phi-4-mini (Metal)")
- [ ] If AI not available, show brief "AI: Offline — conversations unavailable" toast after load
- [ ] If AI loaded successfully, show brief "AI: Ready" toast
- [ ] Loading screen progress bar accounts for AI initialization step
- [ ] Typecheck passes
- [ ] Verify in browser using dev-browser skill
...
**Acceptance Criteria:**
- [ ] When `aiProvider: 'local'` is selected in the export dialog, the export ZIP includes: GGUF model file (~2GB), Piper voice model(s) (~50-100MB each), Whisper model (~150MB)
- [ ] `ai_config.json` in the export has `localModelPath` pointing to the correct relative path (e.g., `ai/models/phi-4-mini-q4.gguf`)
- [ ] Model files are placed in `public/ai/models/` (web) or bundled alongside `dist/` (Electron)
- [ ] Export dialog shows estimated size increase when "Local AI" is selected
- [ ] Export succeeds and models are loadable by the Electron AI service
- [ ] Typecheck passes
