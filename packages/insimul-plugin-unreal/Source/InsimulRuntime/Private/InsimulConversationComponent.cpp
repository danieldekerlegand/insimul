...
void UInsimulConversationComponent::InitializeInsimul()
{
	...
	bOfflineMode = Settings ? Settings->bOfflineMode : false;

	if (bOfflineMode)
	{
		// ── Offline mode: local LLM + exported world data ──
		if (!OfflineProvider)
		{
			OfflineProvider = new FInsimulOfflineProvider();
			OfflineProvider->Initialize(
				Settings->OfflineLLMServerURL,
				Settings->OfflineLLMModel,
				Settings->OfflineMaxTokens,
				Settings->OfflineTemperature
			);
			...
			// Load world data
			OfflineProvider->LoadWorldData(Settings->OfflineWorldDataPath);
		}

		UE_LOG(LogTemp, Log, TEXT("InsimulConversationComponent initialized OFFLINE for character %s"),
			*Config.CharacterID);
	}
	else
	{
		...
		UE_LOG(LogTemp, Log, TEXT("InsimulConversationComponent initialized ONLINE for character %s in world %s"),
