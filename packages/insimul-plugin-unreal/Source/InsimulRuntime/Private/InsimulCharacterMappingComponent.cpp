...
void UInsimulCharacterMappingSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	// Auto-load characters based on mode
	if (Settings && Settings->bOfflineMode)
	{
		// Offline: load from exported JSON file
		LoadInsimulCharactersFromFile(Settings->OfflineWorldDataPath);
	}
	else if (Settings)
	{
	...
	UE_LOG(LogTemp, Log, TEXT("InsimulCharacterMappingSubsystem initialized (world: %s, mode: %s)"),
		*CurrentInsimulWorldId, (Settings && Settings->bOfflineMode) ? TEXT("offline") : TEXT("online"));
}
