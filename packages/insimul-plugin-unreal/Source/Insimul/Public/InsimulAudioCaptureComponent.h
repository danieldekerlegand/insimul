 ...
 *
 * Attach alongside UInsimulChatbotComponent on the player character.
 */
UCLASS(ClassGroup = (Insimul), meta = (BlueprintSpawnableComponent))
class INSIMUL_API UInsimulAudioCaptureComponent : public UActorComponent
{
    GENERATED_BODY()
...
public:
    ...
    FOnInsimulError OnCaptureStarted;

    /** Fired when audio capture stops */
    UPROPERTY(BlueprintAssignable, Category = "Insimul|Audio|Events")
    FOnInsimulError OnCaptureStopped;
