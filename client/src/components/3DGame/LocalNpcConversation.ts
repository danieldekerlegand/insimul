...
import type { ConversationResult } from './NPCSocializationController';
...
export async function generateLocalNpcConversation(
  ...
  if (!LocalAIClient.isAvailable()) return null;
  ...
  try {
    ...
    const response = await LocalAIClient.generate(prompt, systemPrompt, {
      temperature: 0.8,
      maxTokens: 512,
    });
