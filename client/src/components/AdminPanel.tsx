...
import { ArrowLeft, Globe, Layers, BookOpen, Eye, Lock, Calendar, Hash, ChevronRight, ChevronDown, Search, Info, RefreshCw } from "lucide-react";
...
type Section = 'worlds' | 'assets' | 'rules';
...
export function AdminPanel({ onBack }: AdminPanelProps) {
  ...
  const sections: { id: Section; label: string; icon: React.ReactNode }[] = [
    ...
    { id: 'rules', label: 'Rules & Actions', icon: <BookOpen className="w-3.5 h-3.5" /> },
  ];
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-primary/5 p-4">
      <div className="max-w-[1600px] mx-auto space-y-3">
        ...
        {activeSection === 'rules' && <AdminRulesActionsHub />}
      </div>
    </div>
  );
}
