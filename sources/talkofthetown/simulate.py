...
print("\nIt is now the {date}, in the town of {town}, pop. {population}.\n".format(
    ...
    town=sim.city.name,
    population=sim.city.population
))
...
def outline_businesses():
    ...
    print('\nFormer businesses in {town}:'.format(town=sim.city.name))
    for c in sim.city.former_companies:
        print('\t{}'.format(c))
    print('\nCurrent businesses in {town}:'.format(town=sim.city.name))
    for c in sim.city.companies:
        print('\t{}'.format(c))


def outline_character_locations():
    """Outline the locations in town, and who is currently at each one."""
    for location in sim.city.companies|sim.city.dwelling_places:
        print(location)
        if not location.people_here_now:
            ...
            for character in location.people_here_now:
                if character.routine.working:
                    print("\t{} (working as {})".format(character, character.occupation.vocation))
                else:
...
def outline_character_social_network(person):
    ...
    for resident in sim.city.residents:
