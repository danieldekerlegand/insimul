        ...
        if hasattr(self, "subjects"):
            if len(self.subjects) > 1:
                return {
                    "category": "event",
                    "type": event_type,
                    "first": self.subjects[0].full_name,
                    "second": self.subjects[1].full_name,
                    "value": True
                }
            else:
                return {
                    "category": "event undirected",
                    "type": event_type,
                    "first": self.subjects[0].full_name,
                    "value": True
                }
...
class Demolition(Event):
    ...
    def __init__(self, building, demolition_company, reason=None):
        ...
        if building.__class__.__name__ is 'House':
            ...
            self.city.former_dwelling_places.add(building)
        ...
        if building.__class__.__name__ is 'ApartmentComplex':
            for unit in building.units:
                ...
                self.city.former_dwelling_places.add(unit)
                if unit.residents:
