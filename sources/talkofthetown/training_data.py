...
class TrainingDataWriter(object):
    ...
    def _init_parse_json_grammar_specification(self, path_to_json_grammar_specification):
        ...
        for tag, nonterminal_symbol_specification in nonterminal_symbol_specifications.items():
    ...
    def write_out_pickled_symbol_and_token_serialization_dictionaries(self):
        ...
        nonterminal_symbol_to_serialization_id = {
            self.nonterminal_symbols[i].tag: i for i in range(len(self.nonterminal_symbols))
        }
    ...
    def _parse_and_tokenize_terminal_symbols(terminal_symbols):
        ...
        period_regular_expression = re.compile(r'(\w)\.')  # Periods can occur in ellipses, so need special detection
        runtime_variable_regular_expression = re.compile(r'(\[.*\])')
    ...
    def produce_and_write_out_traces(self):
        ...
        for i in range(TARGETED_NUMBER_OF_TERMINAL_DERIVATIONS):
