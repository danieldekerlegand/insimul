...
class Condition(object):
    ...
    def evaluate(self, state):
        ...
        if state.__class__.__name__ == 'Conversation':
...
class ThoughtGenerator(Productionist):
    ...
    def target_association(self, thinker, stimuli):
        ...
        if self.debug:
            print("Attempting to elicit thought given the stimuli: {stimuli}...".format(
                stimuli=', '.join("{signal} ({strength})".format(
                    signal=signal, strength=strength) for signal, strength in stimuli.items()
                )
            ))
        ...
        markup_lambda_expression = (
            lambda symbol: {pair[0] for pair in symbol.signals} & {pair[0] for pair in self.stimuli.items()}
        )
    ...
    def evaluate_nonterminal_symbol(self, nonterminal_symbol):
        ...
        score = 0
        for stimulus_signal, stimulus_signal_weight in self.stimuli.items():
