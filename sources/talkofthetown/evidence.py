...
class PieceOfEvidence(object):
    ...
    def ensemble(self):
        if hasattr(self, "eavesdropper") and self.eavesdropper is not None:
            return {
                "category": "event",
                "type": self.type,
                "first": self.eavesdropper.full_name,
                "second": self.source.full_name,
                "value": True
            }
        elif hasattr(self, "source") and self.source is not None:
            if hasattr(self, "recipient") and self.recipient is not None:
                return {
                    "category": "event",
                    "type": self.type,
                    "first": self.source.full_name,
                    "second": self.recipient.full_name,
                    "value": True
                }
            else:
                return {
        ...
        elif hasattr(self, "subject") and self.subject is not None:
            return {
                "category": "event",
                "type": self.type,
                "first": self.subject.full_name,
                "value": True
            }
        else:
            return None
