...
var addProcessedRules = function(ruleType, fileName, rules) {
	...
	for (i = 0; i < rules.length; i++) {
		...
		if(!rule.id){
			const newId = ruleType + "_" + Random.id();
			ids.push(newId);
			rule.id = newId;
		} else {
			ids.push(rule.id);
		...
		// NOTE!!! Removed following old way of generating rule ids, which does not
		// guarantee unique rule ids and causes trouble when collaborating with others.
...
*@return {Number | Boolean} the value of the specified type between the specified characters. Could either be a number of boolean, as the value might be referring to a boolean type or a numeric one.
*/
...
var ensembleInterface = {
	...
	getStats				: socialRecord.getStats,
...
export default ensembleInterface;