...
var setStats = function(category, type, first, second, isReciprocal) {
	// const key = category + "_" + type;
	const key = type;
	stats[key] = stats[key] ? stats[key] + 1 : 1;
	
	let firstObj = stats["first"][first];
	if (!firstObj) {
		firstObj = {};
	}

	firstObj[key] = firstObj[key] ? firstObj[key] + 1 : 1;
	stats["first"][first] = firstObj;

	if (second) {
		let secondObj = stats["second"][second];
		if (!secondObj) {
			secondObj = {};
		}

		secondObj[key] = secondObj[key] ? secondObj[key] + 1 : 1;
		stats["second"][second] = secondObj;

		if (isReciprocal) {
			let both = stats["both"][`${first}_${second}`];
			if (!both) {
				both = {};
			}
	
			both[key] = both[key] ? both[key] + 1 : 1;
			stats["both"][`${first}_${second}`] = both;
		}
	}
}
...
*/
var set = function(setPredicate, shouldUpdateStats) {
	var pattern = {};
	...
	if (shouldUpdateStats) {
		setStats(setPredicate.category, setPredicate.type, setPredicate.first, setPredicate.second, isReciprocal);

		if (isReciprocal) {
			setStats(setPredicate.category, setPredicate.type, setPredicate.second, setPredicate.first, isReciprocal);
		}
	}
}
...
export default socialRecordInterface;