...
import socialRecord from "./socialRecord.js";
 ...
 var matchUniqueBindings = function (uniqueBindings, availableCastMembers, processResult, rule, params, unaffectedCharacters) {
	...
	if (isFilled === true) {
		...
		if (conditionsAreTrue === true){
			...
			//counter++;
			//console.log("FILLED BINDINGS BUT CONDITIONS WEREN'T TRUE! la la la having fun!" , counter);
		}
	}
	// time to recurse
	else {
		...
		for (var i = 0 ; i < availableCastMembers.length ; i +=1) {
			...
			//BEGIN INTELLIGENT BINDING PROCESS
			var latestKey = {};
			latestKey[emptyKey] = availableCastMembers[i];


			//do our sanity check that what we are working with is actually good.
			var relevantPredicates = doPartialBinding(latestKey, util.clone(rule.conditions));

			//double check that there is at least a *chance* that these conditions will be satisfied...
			//One point of weirdness: this ADDS "weird" predicates to the social record at present
			//e.g., Jacob affinity < 60 with NO second. This is kind of exactly what we want in terms of the sanity
			//checking, but at present the system "helps" by adding the result to the sfdb after this, but maybe
			//we don't even want to do that?
			var isThisBindingOK = true;
			for(var predicateSanityIndex = 0; predicateSanityIndex < relevantPredicates.length; predicateSanityIndex++){
				var tempResult = socialRecord.get(relevantPredicates[predicateSanityIndex], 0, 0, true, {suppressSocialRecord: true});
				if(tempResult.length > 0){
					// so far so good!
					//console.log("binding not violated yet...");
				}
				else{
					//console.log("bad binding! Skip!");
					isThisBindingOK = false;
					break;
				}
			}
			if(!isThisBindingOK){
			...
			//END INTELLIGENT BINDING PROCESS
 ...
 *
 * @method doPartialBinding
 * @memberof RuleLibrary
 * @param {Object} character	a dictionary of a single character to bind to predicates
 * @param {Object} predicates	a clone of the array of predicates that needs characters assigned to each of its roles
 * @return {Array} resultsArray an array of the conditions that have characters bound to their roles. Only contains predicates that pertain to this character.
 */
 var doPartialBinding = function (character, predicates){
	var resultsArray = [];	//array to hold our conditions that have characters bound to roles in each entry
	for (var i = 0 ; i < predicates.length ; i += 1) {
		var predicate = predicates[i];	// current predicate to consider


		if(character[predicate.first] != undefined){
			predicate.first = character[predicate.first];	// bind a character to the role that only had a place-holder
			predicate.second = undefined; // controversial? But will maybe help with quick and dirty tests...?
			resultsArray.push(predicate);
		}
...
export default ruleLibraryInterface;