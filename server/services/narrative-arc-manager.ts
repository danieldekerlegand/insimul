...
export class NarrativeArcManager {
  ...
  async refreshArcProgress(
    ...
    await storage.updateQuest(rootQuest.id, {
      progress: arc as any,
    ...
    for (const act of arc.acts) {
      for (const chapter of act.chapters) {
        ...
        for (let i = 0; i < chapter.subQuestIds.length; i++) {
          ...
          if (prevCompleted) {
            await storage.updateQuest(sqId, { status: 'active' });
  ...
  async getCurrentChapterQuests(
    ...
    for (const sqId of chapter.subQuestIds) {
      const q = await storage.getQuest(sqId);
      if (q) quests.push(q);
    }
  ...
  async deleteArc(worldId: string, playerName: string): Promise<boolean> {
    ...
    await storage.deleteQuest(rootQuest.id);
    return true;
  }
...
// Export singleton
export const narrativeArcManager = new NarrativeArcManager();
