...
export class MongoStorage implements IStorage {
  ...
  async updateAssessmentPhaseResult(sessionId: string, phaseResult: PhaseResult): Promise<AssessmentSession | undefined> {
    await this.connect();
    // Upsert: update existing phase result or push new one
    ...
    if (idx >= 0) {
      update = await AssessmentSessionModel.findByIdAndUpdate(
        sessionId,
        { $set: { [`phaseResults.${idx}`]: phaseResult, updatedAt: new Date().toISOString() } },
        { new: true }
      );
    } else {
      update = await AssessmentSessionModel.findByIdAndUpdate(
        sessionId,
        { $push: { phaseResults: phaseResult }, $set: { updatedAt: new Date().toISOString() } },
        { new: true }
      );
    }
